...
import com.ultrabar.plugin.callback.*;
...
public class PluginClient {
    ...
    public void setRegisterConfig(RegisterPayload rp) {
        this.registerConfig = rp;
    }

    public void setActionsConfig(ActionsPayload ap) {
    ...
    private class ClientHandler extends SimpleChannelInboundHandler<String> {
        ...
        protected void channelRead0(ChannelHandlerContext ctx, String msg) {
            try {
                System.out.println("接受到服务端推送消息:" + msg);
                ...
                if (tryCompletePending(reqId, type, payload)) return;
                ...
                switch (type) {
                    ...
                    case "call":
                        ...
                        return;
                    case "get_options":
                        handleOptions(ctx,reqId,payload);
                        return;
                    default:
        ...
        private void handleDescribe(ChannelHandlerContext ctx, String reqId, JsonNode payload) {
            try {
                ...
                if (pluginListener != null) {
                    submitCallback(() -> {
                        try {
                            pluginListener.onDescribe(dp, responder);
                        } catch (Exception ex) {
        ...
        private void handleCall(ChannelHandlerContext ctx, String reqId, JsonNode payload) {
            try {
                ...
                if (pluginListener != null) {
                    submitCallback(() -> {
                        try {
                            pluginListener.onCall(cp, responder);
                        } catch (Exception ex) {
                            responder.sendError("HANDLER_EXCEPTION", ex.getMessage(), false, null);
                        }
                    });
        ...
        private void handleOptions(ChannelHandlerContext ctx, String reqId, JsonNode payload) {
            try {
                GetOptionsPayload cp = mapper.treeToValue(payload, GetOptionsPayload.class);
                OptionsResponder responder = new OptionsResponder(ctx.channel(), reqId, mapper);
                if (pluginListener != null) {
                    submitCallback(() -> {
                        try {
                            pluginListener.onOptions(cp,responder);
                        } catch (Exception ex) {
                            responder.sendError("HANDLER_EXCEPTION", ex.getMessage(), false, null);
                        }
                    });
